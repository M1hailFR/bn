export const intro = {
  headline: "Мы первые в рейтингах",
  title: "Банкротство и Юридические услуги",
  description:
    "Оставьте заявку на сайте или позвоните нам. Наши юристы внимательно изучат вашу ситуацию и предоставят консультацию.",
  consultation: "Бесплатная консультация",
  start: "Оставить заявку",
  stats: [
    { title: "сотрудников", symbol: "+", value: 990 },
    { title: "помогли", symbol: "+", value: 4980 },
    { title: "офисов", symbol: "", value: 254 },
  ],
};

export const info = {
  title: "Несколько фактов о нас",
  cards: [
    {
      icon: "chat",
      image: "",
      type: "primary",
      title: "Бесплатные консультации",
      text: "Все наши консультационные услуги бесплатны. Мы готовы ответить на все ваши вопросы максимально оперативно и подробно, а также предоставляем постбанкротную поддержку. Вы будете осведомлены о каждом нашем успехе в суде в тот же день.",
    },
    {
      icon: "fire",
      image: "/images/main/work.jpg",
      type: "outline",
      title: "Гарантия возврата средств",
      text: "Гарантируем полный возврат средств, если не поможем в решении вашего вопроса. Банкротство физических лиц без риска: либо результат, либо деньги обратно. Работаем честно и открыто — ваши финансы в безопасности.",
    },
    {
      icon: "justice",
      image: "/images/main/hall.jpg",
      type: "outline",
      title: "Работаем законно",
      text: "Работаем строго в рамках закона, обеспечивая полное соблюдение ваших прав. Только легальные решения и прозрачный подход на каждом этапе сотрудничества.",
    },
    {
      icon: "renew",
      image: "",
      type: "primary",
      title: "Оплатим ваши долги",
      text: "Погасим ваши долги законным путем. Поможем избавиться от финансовой нагрузки, действуя честно, открыто и в рамках законодательства.",
    },

    {
      icon: "notification",
      image: "",
      type: "primary",
      title: "Работаем по всей России",
      text: "Оказываем помощь в банкротстве физических лиц по всей России. Независимо от вашего региона, мы работаем законно, эффективно и дистанционно.",
    },

    {
      icon: "secure",
      image: "/images/main/assembly.jpg",
      type: "outline",
      title: "Доверьте нам ведение вашего дела ",
      text: "Чтобы законно и профессионально решить финансовые проблемы. Мы обеспечим полный контроль над процессом, индивидуальный подход и прозрачность на каждом этапе. С нами вы можете быть уверены в результате и своей финансовой безопасности.",
    },
  ],
};

export const steps = {
  title: "Простые шаги к жизни без долгов",
  cards: [
    {
      index: "01",
      type: "outline",
      title: "Свяжитесь с нами",
      text: "Все наши консультационные услуги бесплатны. Мы готовы ответить на все ваши вопросы максимально оперативно и подробно, а также предоставляем постбанкротную поддержку. Вы будете осведомлены о каждом нашем успехе в суде в тот же день.",
    },
    {
      index: "02",
      type: "outline",
      title: "Подпишите договор",
      text: "Заключите официальный договор и получите законную защиту ваших интересов. Мы работаем открыто, прозрачно и соблюдаем все юридические нормы.",
    },
    {
      index: "03",
      type: "outline",
      title: "Доверьте нам ведение вашего дела",
      text: "Чтобы законно и профессионально решить финансовые проблемы. Мы обеспечим полный контроль над процессом, индивидуальный подход и прозрачность на каждом этапе. С нами вы можете быть уверены в результате и своей финансовой безопасности.",
    },
    {
      index: "04",
      type: "outline",
      title: "Мы подадим документы в суд",
      text: "Мы подадим все необходимые документы в суд и обеспечим правильное сопровождение процесса. Наши юристы гарантируют соблюдение всех юридических норм и защиту ваших интересов на каждом этапе.",
    },

    {
      index: "05",
      type: "outline",
      title: "Добьёмся несостоятельности",
      text: "Мы добьемся признания вашей несостоятельности в соответствии с законом, обеспечив полное юридическое сопровождение и защиту ваших прав на каждом этапе процесса.",
    },

    {
      index: "06",
      type: "outline",
      title: "Завершим списанием ваших долгов",
      text: "Мы успешно завершим процесс списания ваших долгов, обеспечивая легальный и надежный подход к решению финансовых вопросов.",
    },
  ],
};
export const servise = {
  title: "Кому подходит услуга",
  icon: "secure",
  image: "/images/main/assembly.jpg",
  text: "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Lorem, ipsum dolor sit amet consectetur adipisicing elit.",
  cards: [
    {
      icon: "notification",
      type: "outline",
      title: "Свяжитесь с нами",
      text: "Lorem, ipsum dolor sit amet consectetur adipisicing elit.",
    },
    {
      icon: "notification",
      type: "outline",
      title: "Подпишите договор",
      text: "Lorem, ipsum dolor sit amet consectetur adipisicing elit.",
    },
    {
      icon: "notification",
      type: "outline",
      title: "Доверьте нам ведение вашего дела",
      text: "Lorem, ipsum dolor sit amet consectetur adipisicing elit.",
    },
    {
      icon: "notification",
      type: "outline",
      title: "Мы подадим документы в суд",
      text: "Lorem, ipsum dolor sit amet consectetur adipisicing elit.",
    },

    {
      icon: "notification",
      type: "outline",
      title: "Добьёмся несостоятельности",
      text: "Lorem, ipsum dolor sit amet consectetur adipisicing elit.",
    },
    {
      icon: "notification",
      type: "outline",
      title: "Мы подадим документы в суд",
      text: "Lorem, ipsum dolor sit amet consectetur adipisicing elit.",
    },

    {
      icon: "notification",
      type: "outline",
      title: "Добьёмся несостоятельности",
      text: "Lorem, ipsum dolor sit amet consectetur adipisicing elit.",
    },
    {
      icon: "notification",
      type: "outline",
      title: "Добьёмся несостоятельности",
      text: "Lorem, ipsum dolor sit amet consectetur adipisicing elit.",
    },
  ],
};

export const achievement = {
  title: "Кому подходит услуга",
  icon: "secure",
  image: "/images/main/assembly.jpg",
  cards: [
    {
      icon: "notification",
      type: "primary",
      title: "Свяжитесь с нами",
      text: "Lorem, ipsum dolor sit amet consectetur adipisicing elit.",
    },
    {
      icon: "notification",
      type: "primary",
      title: "Подпишите договор",
      text: "Lorem, ipsum dolor sit amet consectetur adipisicing elit.",
    },
    {
      icon: "notification",
      type: "primary",
      title: "Доверьте нам ведение вашего дела",
      text: "Lorem, ipsum dolor sit amet consectetur adipisicing elit.",
    },
    {
      icon: "notification",
      type: "primary",
      title: "Мы подадим документы в суд",
      text: "Lorem, ipsum dolor sit amet consectetur adipisicing elit.",
    },

    {
      icon: "notification",
      type: "primary",
      title: "Добьёмся несостоятельности",
      text: "Lorem, ipsum dolor sit amet consectetur adipisicing elit.",
    },
    {
      icon: "notification",
      type: "primary",
      title: "Мы подадим документы в суд",
      text: "Lorem, ipsum dolor sit amet consectetur adipisicing elit.",
    },

    {
      icon: "notification",
      type: "primary",
      title: "Добьёмся несостоятельности",
      text: "Lorem, ipsum dolor sit amet consectetur adipisicing elit.",
    },
    {
      icon: "notification",
      type: "primary",
      title: "Добьёмся несостоятельности",
      text: "Lorem, ipsum dolor sit amet consectetur adipisicing elit.",
    },
  ],
};

export const reviews = {
  title: "Отзывы клиетов",
};

export const heros = [
  {
    type: "primary",
    title: "Lorem, ipsum dolor sit amet consectetur adipisicing elit.",
    text: "Lorem, ipsum dolor sit amet consectetur adipisicing elit. In veniam ut voluptatibus officiis, inventore facilis non incidunt error officia explicabo ex fugit ducimus itaque delectus quia illo aperiam esse minima.",
    image: "/images/main/work.jpg",
  },
  {
    type: "default",
    title: "Lorem, ipsum dolor sit amet consectetur adipisicing elit.",
    text: "Lorem, ipsum dolor sit amet consectetur adipisicing elit. In veniam ut voluptatibus officiis, inventore facilis non incidunt error officia explicabo ex fugit ducimus itaque delectus quia illo aperiam esse minima.",
    image: "/images/main/work2.jpg",
  },
  {
    type: "video",
    title: "Lorem, ipsum dolor sit amet consectetur adipisicing elit.",
    text: "Lorem, ipsum dolor sit amet consectetur adipisicing elit. In veniam ut voluptatibus officiis, inventore facilis non incidunt error officia explicabo ex fugit ducimus itaque delectus quia illo aperiam esse minima.",
    video: "/videos/intro2.mp4",
    stats: [
      { title: "сотрудников", symbol: "+", value: 990 },
      { title: "помогли", symbol: "+", value: 99980 },
      { title: "офисов", symbol: "", value: 254 },
      { title: "сотрудников", symbol: "+", value: 9904 },
      { title: "помогли", symbol: "+", value: 49 },
      { title: "Списанно мл. р", symbol: "+", value: 25 },
    ],
  },
];

export const fqa = {
  title: "Часто задаваемые вопросы",
  description:
    "Have a project in mind? Let's connect and see how we can collaborate. Reach out to us for inquiries or just to say hello!",
  questions: [
    {
      title: "Что такое юридический трафик и зачем он нужен?",
      text: "Юридический трафик — это поток потенциальных клиентов, которые ищут юридические услуги. Он помогает компаниям увеличивать количество заявок и привлекать новых клиентов.",
    },
    {
      title: "Что такое лид?",
      text: "Лид — это потенциальный клиент, который проявил интерес к вашему продукту или услуге и оставил свои контактные данные для дальнейшего взаимодействия. Лиды могут быть получены через различные каналы: сайт, рекламные кампании, социальные сети и т.д. Основная задача компании — преобразовать лидов в реальных клиентов, используя подходящие маркетинговые или продажные стратегии.",
    },
    {
      title: "Вы можете предоставить мне лиды только по уголовным делам?",
      text: "Нет, к сожалению мы предоставляем лиды только общего спектра юридических услуг, там будут заявки и на социальные вопросы, на земельные, уголовные, административные и т.д",
    },
    {
      title: "Какие источники трафика вы используете для привлечения клиентов?",
      text: "Мы используем поисковую оптимизацию, контекстную рекламу, социальные сети и партнерские программы для привлечения юридически заинтересованных пользователей.",
    },
    {
      title: "Что такое юридический трафик и зачем он нужен?",
      text: "Юридический трафик — это поток потенциальных клиентов, которые ищут юридические услуги. Он помогает компаниям увеличивать количество заявок и привлекать новых клиентов.",
    },
    {
      title: "Что такое лид?",
      text: "Лид — это потенциальный клиент, который проявил интерес к вашему продукту или услуге и оставил свои контактные данные для дальнейшего взаимодействия. Лиды могут быть получены через различные каналы: сайт, рекламные кампании, социальные сети и т.д. Основная задача компании — преобразовать лидов в реальных клиентов, используя подходящие маркетинговые или продажные стратегии.",
    },
    {
      title: "Вы можете предоставить мне лиды только по уголовным делам?",
      text: "Нет, к сожалению мы предоставляем лиды только общего спектра юридических услуг, там будут заявки и на социальные вопросы, на земельные, уголовные, административные и т.д",
    },
    {
      title: "Какие источники трафика вы используете для привлечения клиентов?",
      text: "Мы используем поисковую оптимизацию, контекстную рекламу, социальные сети и партнерские программы для привлечения юридически заинтересованных пользователей.",
    },
    {
      title: "Что такое юридический трафик и зачем он нужен?",
      text: "Юридический трафик — это поток потенциальных клиентов, которые ищут юридические услуги. Он помогает компаниям увеличивать количество заявок и привлекать новых клиентов.",
    },
    {
      title: "Что такое лид?",
      text: "Лид — это потенциальный клиент, который проявил интерес к вашему продукту или услуге и оставил свои контактные данные для дальнейшего взаимодействия. Лиды могут быть получены через различные каналы: сайт, рекламные кампании, социальные сети и т.д. Основная задача компании — преобразовать лидов в реальных клиентов, используя подходящие маркетинговые или продажные стратегии.",
    },
    {
      title: "Вы можете предоставить мне лиды только по уголовным делам?",
      text: "Нет, к сожалению мы предоставляем лиды только общего спектра юридических услуг, там будут заявки и на социальные вопросы, на земельные, уголовные, административные и т.д",
    },
    {
      title: "Какие источники трафика вы используете для привлечения клиентов?",
      text: "Мы используем поисковую оптимизацию, контекстную рекламу, социальные сети и партнерские программы для привлечения юридически заинтересованных пользователей.",
    },
  ],
};
