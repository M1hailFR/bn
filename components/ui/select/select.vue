    ...
    <div
      ...
      <template v-if="searchable && isOpen">
        <input
          ...
          type="text"
          ref="SearchElement"
          class="w-full h-4 bg-transparent outline-0 border-0" />
      </template>
      ...
      <Icon
        class="relative w-4 h-4 ml-auto transition group-hover:top-0.5"
...
.select-component {
  .placeholder {
    @apply text-white-base/50;
  }
  &.type {
    &-primary {
      @apply text-white-base bg-gray-light border-l-4 border-green-base rounded;
      &.opened {
        @apply rounded-none rounded-t;
      }
      .select-component-options {
        @apply bg-gray-light rounded-b border-l-4 border-green-base -left-1 w-[calc(100%+4px)];
        .option:hover,
        .option.choose {
          @apply bg-white-soft/20;
        }
      }
    }
  }
