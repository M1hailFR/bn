...
.button-component {
  ...
  &.type {
    &-primary {
      @apply rounded-full bg-primary text-white hover:bg-gradient-primary;
    }
    &-outline {
      @apply rounded-full border-white/80 md:border-gray-100 border  hover:bg-gradient-primary hover:text-white hover:border-gray-400;
    }
    &-secondary {
      @apply relative text-white rounded-full overflow-hidden hover:bg-gradient-primary hover:text-white;
      &::after {
        content: '';
