...
<template>
  <menu class="flex items-center gap-4">
    <li
      ...
      :class="`i-${theme}`"
    >
      <Link :href="item.href" type="default">
        <p>{{ item.title }}</p>
      </Link>
        ...
        <div class="sub-level-container">
          <div class="flex w-full">
            <div
              ...
              :key="index"
              class="flex flex-col border-r-2 pr-6 pl-5 first:pl-2 last:border-0 border-gray-900/20"
            >
              <Link
                :href="chls.href"
              ...
              <Link
                ...
                class="flex items-center justify-start gap-0.5 border-b border-gray-900 mt-10 pl-1"
                type="primary"
              >
                <Icon v-if="chls.icon" :name="chls.icon" size="normal" />
                <h6 class="font-bold mt-1">
                  {{ chls.buttonTitle }}
                </h6>
              </Link>
            </div>
...
.level-item {
  ...
  &:hover {
    ...
    .sub-level {
      @apply visible opacity-100 bg-gray-100/90;
    }
  }
}
.sub-level {
